...
export default function CertificationPage() {
  ...
  // ✅ Par défaut, on affiche la dernière campagne (la plus récente)
  useEffect(() => {
    if (selectedBatchId === null && certBatches.length > 0) {
      const latest = [...certBatches].sort(
        (a, b) => new Date(b.launched_at).getTime() - new Date(a.launched_at).getTime()
      )[0];
      setSelectedBatchId(latest.id);
    }
  }, [certBatches, selectedBatchId]);
  ...
  useEffect(() => {
    if (pollData?.status === "DONE" || pollData?.status === "FAILED") {
      ...
      if (pollData.status === "DONE") {
        ...
        setTimeout(() => setShowImportModal(false), 600);
      } else {
  ...
  const importBusy = uploadMut.isPending || isRunning ||
    (importResult != null && importResult.taskStatus !== "SUCCESS" && importResult.taskStatus !== "FAILURE");
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F3]">
      <main className="px-6 py-6 space-y-4">
        ...
        {/* ── Header simple (même design que Tableau de bord / Suivi Facturation) ── */}
        <div className="bg-white rounded-[20px] shadow-[0_1px_3px_rgba(15,23,42,.04),0_8px_28px_rgba(15,23,42,.06)]">
          <div className="px-6 py-5 flex items-start justify-between gap-3 flex-wrap">
            <div>
              <h1 className="text-[22px] font-extrabold text-slate-900 tracking-tight leading-tight">Certification</h1>
              <p className="text-[13px] text-slate-500 mt-1">Validation Sénélec × FMS Grid/ACM · statuts, alertes mesure et export.</p>
            </div>
            <div className="flex items-center gap-3">
              <EfmsDot reachable={efms?.efms_reachable} loading={efmsLoading} />
              <button onClick={openImportModal}
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-950 text-white font-bold text-[13px] rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-950/20">
                <Upload className="w-3.5 h-3.5" /> Importer une facture
              </button>
            </div>
          </div>
        </div>

        {/* ── Liste des campagnes + résultats ── */}
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.04)] px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={openImportModal} className="flex items-center gap-1 text-[11px] font-semibold text-slate-600 px-3 py-1.5 rounded-lg hover:bg-slate-100 border border-slate-200 transition">
      ...
      {/* ── Modal import + certification ── */}
      <Dialog open={showImportModal} onOpenChange={(next) => { if (!next) setShowImportModal(false); }}>
        <DialogContent
          className="p-0 gap-0 border-0 max-w-2xl"
        ...
        >
          <div className="px-6 pt-5 pb-4 border-b border-slate-100">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <DialogTitle asChild>
                <div className="text-[16px] font-bold text-slate-900">Importer une facture</div>
              </DialogTitle>
            </div>
            <StepBar current={step === "results" ? "certify" : step} />
          </div>
          ...
          <div className="p-6 space-y-4">
            ...
            {/* ── sous-étape : certify ── */}
            {(step === "certify" || step === "results") && importResult && (
              <div className="space-y-4">
                {/* fichier importé */}
